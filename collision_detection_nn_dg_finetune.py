...
# def get_mean_error_hypothesis(sess, x_test, y_test, keep_prop=1.0, is_train=False):
#     return sess.run(hypothesis, feed_dict={X: x_test, Y:y_test,  keep_prob:keep_prop, is_train:is_train})

# def train(sess, x_data, y_data, keep_prop=1.0, is_train=True):
#     return sess.run(hypothesis, feed_dict={X: x_data, Y:y_data,  keep_prob:keep_prop, is_train:is_train})
...
for epoch in range(training_epochs):
    ...
    while True:
        try:
            ...
            if (x.shape[0]==batch_size):
                ...
                # read_finish_time = time.time()
                # print('calc time : ', read_finish_time-read_start_time)

                # read_start_time = time.time()
                # prediction_train = np.argmax(hypo_train, 1)
                # correct_prediction_train = np.equal(prediction_train, np.argmax(y, 1))
                # accuracy_train_local = np.mean(correct_prediction_train)
                # accuracy_train_all = accuracy_train_all + accuracy_train_local
                # cost_train_local = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(targets=y, logits=logits_train, pos_weight=cross_entropy_weight))
                # cost_train_all = cost_train_all + cost_train_local
                # read_finish_time = time.time()
                # print('calc2 time : ', read_finish_time-read_start_time)

                ##Train part
                # l2_reg = sum(tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES))
                # update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
                # with tf.control_dependencies(update_ops):
                #     optimizer = tf.train.AdamOptimizer(learning_rate= learning_rate).minimize(cost_train_local + l2_reg)
            ...
            # print('cost_train_local:', '{:.%9f}'.format(cost_train_local))
    ...
    if wandb_use == True:
        wandb_dict = dict()
        # wandb_dict['Training Accuracy'] = accuracy_train
        wandb_dict['Validation Accuracy'] = accuracy_val
        # wandb_dict['Training Cost'] = cost_train.eval(session = sess)
