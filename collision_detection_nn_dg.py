...
if wandb_use == True:
    wandb.init(project="Dusan_2nd_Project", name="fp_test_with_"+args.input_type+"_ver"+str(args.version), tensorboard=False)
...
TrainData = tf.data.TFRecordDataset(["../data/TrainingData_robot12_"+args.input_type+".tfrecord"])
...
ValidationData = pd.read_csv('../data/ValidationData_robot1_'+args.input_type+'.csv').as_matrix().astype('float64')
...
saver.save(sess,'model/model_test_robot12_'+args.input_type+'_v'+str(args.version)+'.ckpt')

if wandb_use == True:
    saver.save(sess, os.path.join(wandb.run.dir, 'model/model_test_robot12_'+args.input_type+'_v'+str(args.version)+'.ckpt'))
